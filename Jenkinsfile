            ...
            steps {
                ...
                sh '''#!/bin/bash
                    ...
                    then 
                        ...
                        scp -o StrictHostKeyChecking=no -i /root/id_rsa -r ./ root@$(cat /root/tfstate/script-openstack-do.tfstate | jq \'.["outputs"]["ips"]["value"][0]\' | sed \'s|"||g\' ):/root/script-openstack/
                        ssh -o StrictHostKeyChecking=no -i /root/id_rsa root@$(cat /root/tfstate/script-openstack-do.tfstate | jq \'.["outputs"]["ips"]["value"][0]\' | sed \'s|"||g\' ) "cd /root/script-openstack/; ./start.sh"
                    fi 
                '''
                echo 'Finished'
            }
